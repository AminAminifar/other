...
for run_number in range(Number_of_Runs):
    ...
    pd.DataFrame(Data).to_csv('Adult Data Set/CSV/RSA/10run/Adult_sh_' + str(run_number) + '.csv', header=False,
                              index=False)
    ...
    for i in range(0, DSlength_D1):
        for j in range(0, (DSlength_D2 - 1)):
            Data_e[i][j] = publickey.encrypt(long(Data[i][j]), '')[0]
    ...
    for i in range(0, TestDSlength_D1):
        for j in range(0, (TestDSlength_D2 - 1)):
            TestData_e[i][j] = publickey.encrypt(long(TestData[i][j]), '')[0]
    ...
    # pd.DataFrame(Data_e).to_csv("../Adult Data Set/CSV/Adult_Encrypted.csv", header=False, index=False)#ADDRESS
    # pd.DataFrame(TestData_e).to_csv("../Adult Data Set/CSV/Test_Adult_Encrypted.csv", header=False, index=False)#ADDRESS
    #
    # print "Data Saved!"
    ...
    pd.DataFrame(N_E_Data_withLables).to_csv('Adult Data Set/CSV/RSA/10run/Adult_Encrypted_Normalized_'+str(run_number)+'.csv', header=False, index=False)
    pd.DataFrame(N_TestData_e).to_csv('Adult Data Set/CSV/RSA/10run/Test_Adult_Encrypted_Normalized_'+str(run_number)+'.csv', header=False, index=False)
...
pd.DataFrame(Accuracy_E_N).to_csv('Adult Data Set/CSV/RSA/10run/AccuracyTable.csv', header=False, index=False)
